...
def t_create_module(root_dir, module_name, dotted_namespace, funcs, classes):
    ...
    verbose.print_log(f"Generating {current_dir}/{module_name}.rst")
...
def t_create_dir(path, dirname: str, dotted_namespace, root):
    ...
    verbose.print_log(f"Generating {abs_path}")
    with open(abs_path + ".rst", mode="w") as file:
        file.write(write_to_file)


def generate_test_rst():
    verbose.print_log("Generating test docs")
...
def get_index_tree():
    verbose.print_log("Generating index for testing docs")
    files = []
...
def generate():
    ...
    verbose.print_log(f"Generating test docs in {path_to_generate_in}")
