    ...
    if "functional/ivy/" in html_filepath:
        ...
        for i in range(len(function_defs)):
            ...
            function_call_indices = [
                ...
                or docstring_content.startswith("for ivy.", i)
            ]
            function_calls = []
            for index in function_call_indices:
                if docstring_content[index] == "f":
                    function_calls.append(
                        (docstring_content[index + 4 :].split()[0].strip("."), False)
                    )
                else:
                    function_calls.append(
                        (docstring_content[index + 3 :].split()[0].strip("."), True)
                    )
            ...
            for function_call, flag in function_calls:
                ...
                final_path = '<a href="{}./{}/{}_functional.html">{}</a>'.format(
                    dot, function_name, function_name, function_call
                )
                if flag:
            ...
            for i in range(len(function_call_indices)):
                if linked_calls[i]:
                    docstring_content = (
                        docstring_content[0 : function_call_indices[i]]
                        + linked_calls[i]
                        + " "
                        + " ".join(
