...
def create_index_rst(sub_contents_dict):
    ...
    for key, value in sub_contents_dict.items():
        new_key = key.split('/')[-1]
        new_value = [item.split('/')[-1].replace('.py',
                                                 '') + '.rst' for item in value]
        ...
        while i < len(content):
            ...
            if find_def:
                if line[0:8] == "    def " and not inside_nest:
                    while '):' not in line and "ivy.Array:" not in line and "ivy.Container:" not in line:
                        f.write('{}\n'.format(line[4:]))
                        i += 1
                        if i == len(content):
                            return
...
def add_instance_and_static_methods(directory):
    ...
    sub_dirs = [item for item in cont_paths if os.path.isdir(
        item) and item.split('/')[-1] not in EXCLUDED_DIRS]
    ...
    modules = [item for item in cont_paths if item[-3:] ==
               '.py' and item.split('/')[-1][:-3] not in EXCLUDED_MODULES]
    ...
    for module in modules:
        ...
        for i in range(num_rsts_to_create):
            ...
            # Dotted namespace
            dotted_namespace = '/'.join(module[repo_location:-3].split('/')[
                0:i+3]).replace('/', '.')
        ...
        if index != -1:
            add_array_and_container_code(
                module_str, module, dotted_namespace)
...
def get_functions_and_classes(module_path, dotted_namespace):
    ...
    with open(module_path, errors='replace') as file:
        module_str = file.read()
    all_function_names = [dotted_namespace + '.' +
                          item.split('(')[0] for item in module_str.split('\ndef ')[1:]]
    public_function_names = [
        n for n in all_function_names if n.split('.')[-1][0] != '_']
    class_names = [dotted_namespace + '.' +
                   item.split('(')[0] for item in module_str.split('\nclass ')[1:]]
    ...
    for module in modules:
        ...
        for i in range(num_rsts_to_create):
            ...
            new_filepath = os.path.join(
                'autogenerated_source', rel_path).replace('.py', '') + '.rst'
            ...
            new_module_dir = os.path.join(
                'autogenerated_source', rel_path).replace('.py', '')
        ...
        functions, classes = get_functions_and_classes(
            module, dotted_namespace)
        ...
        for func_name, dotted_func in zip(function_names, functions):
            function_filepath = os.path.join(
                new_module_dir, func_name) + '.rst'
            extension = ''
...
def add_instance_and_static_rsts():
    ...
    array_code_path = os.path.join(
        ROOT_DIR, "array", "array_methods.py")
    container_path = os.path.join(
        THIS_DIR, "autogenerated_source", "container", "container_methods")
    container_code_path = os.path.join(
        ROOT_DIR, "container", "container_methods.py")
    ...
    for function in functions:
        ...
        index = [index for index in range(
            len(files)) if 'functional' in files[index]][0]
        temp = files[0]
        files[0] = files[index]
        files[index] = temp
...
def write_header_and_toctree_to_rst(folder_path, file_path, title, module_name, submodule):
    toctree_dict = {}
...
def move_folders_to_classes():
    ...
    classes_path = os.path.join(doc_path, 'classes')
    data_classes_path = os.path.join(classes_path, 'data_classes')
    os.makedirs(classes_path, exist_ok=True)
    shutil.move(os.path.join(doc_path, 'stateful'), classes_path)
    ...
    write_header_and_toctree_to_rst(os.path.join(classes_path, 'stateful'),
                                    os.path.join(classes_path, 'stateful.rst'),
                                    'Framework Classes',
                                    'ivy.stateful',
                                    'stateful')
    update_image_paths(os.path.join(classes_path, 'stateful'), '../../', '../../../')

    write_header_and_toctree_to_rst(os.path.join(classes_path, 'data_classes', 'array'),
                                    os.path.join(
                                        classes_path, 'data_classes', 'array.rst'),
                                    'Array',
                                    'ivy.Array',
                                    'array')

    write_header_and_toctree_to_rst(os.path.join(classes_path, 'data_classes', 'container'),
                                    os.path.join(
                                        classes_path, 'data_classes', 'container.rst'),
                                    'Container',
                                    'ivy.Container',
                                    'container')

    write_header_and_toctree_to_rst(os.path.join(classes_path, 'data_classes'),
                                    os.path.join(
                                        classes_path, 'data_classes.rst'),
                                    'Data Classes',
                                    'ivy.data_classes',
                                    'data_classes')
    ...
    toctree_content = [content for content in toctree_content
                       if ':caption: Array' not in content
                       and ':caption: Container' not in content
                       and ':caption: Stateful' not in content]
    index = [index for index in range(len(toctree_content))
             if ':caption: Functions' in toctree_content[index]][0]
    ...
    files = [file for file in os.listdir(
        os.path.join(classes_path)) if '.rst' in file]
    toctree_dict['classes'] = files
    append_toctree_to_rst(toctree_dict, os.path.join(
        doc_path, 'index.rst'), 'Classes', False)
    ...
    if os.path.exists(submod_orders_path):
        ...
        with open(submod_orders_path, 'r') as f:
            submod_orders = [l.replace('\n', '').replace(
                ' ', '')[1:-1].split(',') for l in f.readlines()[1:]]
