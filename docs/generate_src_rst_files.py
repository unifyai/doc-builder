...
def write_discussion_links():
    verbose.print_log("Writing discussion links")
...
def aggregate_experimental(folder):
    ...
    file_str += SUPPORTED_FRAMEWORKS.replace("logos", "../" * 2 + "logos")
    rst_file = folder + '.rst'
...
def main(root_dir, submodules_title):
    ...
    verbose.print_log(f"Skipping modules in {submods_to_skip_path}")
    ...
    if project_title.lower() == "ivy":
        verbose.print_log("Ivy only is enabled")
        IVY_ONLY = True
    ...
    with open("partial_source/conf.py", "w") as conf_file:
        verbose.print_log("Reading config from partial source")
        conf_file.write(conf_contents)

    # just to remove previously generated rst files
    if os.path.exists("autogenerated_source"):
        verbose.print_log("Removing old autogenerated_source directory")
    ...
    if IVY_ONLY:
        ...
        aggregate_experimental(
            "autogenerated_source/data_classes/container/experimental"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Enable verbose logging for doc-builder"
    )
    parsed_args = parser.parse_args()
    verbose.set_verbose(parsed_args.verbose)
