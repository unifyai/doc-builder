...
# Local
from generate_test_files import generate as t_generate, get_index_tree
...
def create_index_rst(sub_contents_dict):
    ...
    # Write testing docs to index.rst
    toctree_dict["testing"] = get_index_tree()
...
def filter_code(all_code, pattern):
    matches = pattern.finditer(all_code)
    intervals = [list(match.span()) for match in matches]
    setup_code, other_code = "", ""
    start = 0
    if len(intervals):
...
def format_docstring(docstring, method=False):
    spaces = "    "
    if method:
        spaces = "        "
    header_pattern = re.compile(r"{0}.*\n{0}-+\n".format(spaces))
    matches = header_pattern.finditer(docstring)
    offset = 0
    for match in matches:
        start = match.span()[0] + offset
        docstring = docstring[0:start] + "\n" + docstring[start:]
        offset += 1
    example_idx = docstring.find("\n{}Examples".format(spaces))
    if example_idx != -1:
        lines = docstring[example_idx:].split("\n")
        inside_example = False
        first = True
        for i, line in enumerate(lines):
            if re.search(r"{}(With|Using) :.*\..*:".format(spaces), line):
                lines[i] = lines[i] + "\n"
            if line.startswith("{}>>> ".format(spaces)):
                if not inside_example:
                    lines[i] = lines[i] if first else ("\n" + lines[i])
                    inside_example = True
                    first = False
                ...
                inside_example = False
        examples = "\n".join(lines)
        docstring = docstring[:example_idx] + examples
    return docstring


def get_func_after_processing(code, matches, method=False):
...
def add_array_and_container_code(module_str, module_path, dotted_namespace):
    ...
    pattern_priv_func = re.compile(r"def .*\n(    .*(\n)?)*(\):.*\n)?(    .*(\n)?)*")

    # Separate class code, function code, private function code and other code
    setup_code, class_code = filter_code(all_code, pattern_class)
    all_code = setup_code[:]
    setup_code, func_code = filter_code(all_code, pattern_func)
    all_code = setup_code[:]
    setup_code, priv_func_code = filter_code(all_code, pattern_priv_func)
    ...
    with open(file, "a") as f:
        ...
        priv_def_matches = def_pattern.finditer(priv_func_code)
        priv_func_code, _ = get_func_after_processing(priv_func_code, priv_def_matches)
        priv_func_code_lines = priv_func_code.split("\n")
        for line in priv_func_code_lines:
            f.write("{}\n".format(line))
        def_matches = def_pattern.finditer(func_code)
        func_code, _ = get_func_after_processing(func_code, def_matches)
        func_code_lines = func_code.split("\n")
        for line in func_code_lines:
            f.write("{}\n".format(line))

        spaces = "        "
        single_line_method_pattern = re.compile(r"\n    def .*\(.*\):.*\n")
        method_pattern = re.compile(
            r'\n    def .*\n({0}[^"].*[\*:\[,\]].*(\n)?)*(    \).*:(\n)?)?'.format(
                spaces
            )
        )
        single_line_method_matches = list(
            single_line_method_pattern.finditer(class_code)
        )
        single_line_starts = set(
            [match.span()[0] for match in single_line_method_matches]
        )
        method_matches = method_pattern.finditer(class_code)
        method_matches = [
            match
            for match in method_matches
            if match.span()[0] not in single_line_starts
        ]
        matches = single_line_method_matches + method_matches
        matches.sort(key=lambda x: x.span()[0])
        class_code, method_strs = get_func_after_processing(
            class_code, matches, method=True
        )
        method_str_lines = method_strs.split("\n")
        for line in method_str_lines:
            if line.startswith("    "):
                line = line[4:]
            f.write("{}\n".format(line))
...
def get_functions_and_classes(module_path, dotted_namespace):
    ...
    class_names = [
        dotted_namespace + "." + item[: min(item.find("("), item.find(":"))]
        for item in module_str.split("\nclass ")[1:]
    ]
    ...
    while i < n:
        ...
        if "/" in content[i]:
            ...
            content[i] = content[i].replace(
                submodule, "{}/{}".format(submodule, submodule)
            )
        i += 1
    with open(file_path, "w") as f:
...
def write_header_and_toctree_to_rst(
    ...
    if "data_classes/array.rst" in file_path:
        copy_from_file = file_path.replace(
            "data_classes/array.rst", "data_classes/array/array.rst"
        )
        copy_contents_and_update_path(file_path, copy_from_file, submodule)
    elif "data_classes/container.rst" in file_path:
        copy_from_file = file_path.replace(
            "data_classes/container.rst", "data_classes/container/container.rst"
        )
        copy_contents_and_update_path(file_path, copy_from_file, submodule)
    else:
    ...
    files = [
        ...
        if ".rst" in file and file.split("/")[-1] not in ["array.rst", "container.rst"]
    ]
    files.sort()
...
def aggregate_experimental(folder):
    ...
    for file in files:
        with open(file) as f:
            content = f.read()
            auto_class_pattern = re.compile(
                r".. autoclass:: .*\n   :members:\n   :special-members: __init__\n   :undoc-members:\n   :show-inheritance:"
            )
            file_str += auto_class_pattern.findall(content)[0] + "\n\n\n"
    file_str += SUPPORTED_FRAMEWORKS.replace("logos", "../../logos")
    rst_file = folder + '.rst'
...
def main(root_dir, submodules_title):
    # This directory contains all files in the repository along with the permitted_namespaces.json,
    # submods_to_skip.txt and submods_to_step.txt files
    global THIS_DIR
    ...
    def submods_list_init(fname):
        submods_path = os.path.join(THIS_DIR, fname)
        if os.path.exists(submods_path):
    ...
    if os.path.exists(submod_orders_path):
        ...
        submod_orders_sorted = [tuple(sorted(submod_order)) for submod_order in submod_orders]
    ...
    global IVY_ONLY
    IVY_ONLY = project_title.lower() == "ivy"

    # The configuration file is updated with the name of the project
    ...
    if IVY_ONLY:
        ...
        def rm_autogenerated(name):
            shutil.rmtree(f"autogenerated_source/{name}/{name}_methods")
            os.remove(f"autogenerated_source/{name}/{name}_methods.rst")

        rm_autogenerated("container")
        rm_autogenerated("array")

        # Move the stateful, array and container folders inside the classes folder
        move_folders_to_classes()

        aggregate_experimental("autogenerated_source/data_classes/container/experimental")
        aggregate_experimental("autogenerated_source/data_classes/array/experimental")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Root directory of the repository relative to current directory.",
    )
    parser.add_argument(
        "--submodules_title",
        type=str,
